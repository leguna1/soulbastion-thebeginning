...
void UAbilitySystem::UpdateMontagePlayRate(UAnimMontage* MontageRef, FAbilityMontageParams MontageParams, float PlayRateMultiplier)
{
...
void UAbilitySystem::OnMontageEndHandler(UAnimMontage* Montage, bool bInterrupted)
{
	...
	// Clear buffered skill if it was the old one
	if (BufferedSkill && BufferedSkill == OldActiveSkill)
	{
		BufferedSkill = nullptr;
	}
}
